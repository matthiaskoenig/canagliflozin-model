"""Tool to run the canagliflozin model factory and simulation experiments."""
...
from pkdb_models.models.canagliflozin import CANAGLIFLOZIN_PATH
from pkdb_models.models.canagliflozin.simulations import run_simulation_experiments, EXPERIMENTS
from sbmlutils.console import console

FACTORY_SCRIPT_PATH = CANAGLIFLOZIN_PATH / "models" / "factory.py"
...
def _setup_custom_results_paths(results_dir: str):
    ...
    canagliflozin.RESULTS_PATH = custom_path
    canagliflozin.RESULTS_PATH_SIMULATION = custom_path / "simulation"
...
def _get_current_results_path():
    ...
    from pkdb_models.models.canagliflozin import RESULTS_PATH
    return RESULTS_PATH
...
def _list_available_experiments():
    ...
    console.print('[dim]Example: run_canagliflozin --action simulate --experiments "misc,LaCreta2016"[/dim]')
    console.print('[dim]Or use "all" to run all experiments: run_canagliflozin --action simulate --experiments all[/dim]\n')
...
def main() -> None:
    ...
    console.rule("[bold cyan]CANAGLIFLOZIN PBPK/PD MODEL[/bold cyan]", style="cyan")
    ...
    if options.results_dir:
        ...
        if options.action and options.action.lower() == "factory":
            console.print(f"[cyan]Factory will use model base path: {canagliflozin.MODEL_BASE_PATH}[/cyan]")
        else:
...
if __name__ == "__main__":
    """
    This script is intended to be run from the command line as 'run_canagliflozin'.
    ...
    1. Help:
       ...
       $ run_canagliflozin --help

    2. Run the Model Factory:
       Generates all SBML model files.
       $ run_canagliflozin --action factory

    3. Run Simulations:
       List available experiments:
       $ run_canagliflozin --action list_experiments

       Run all experiments:
       $ run_canagliflozin --action simulate --experiments all

    5. Run Everything:
       Runs factory and all simulations.
       $ run_canagliflozin --action all
       
       With custom results directory for figures:
       $ run_canagliflozin --action all --results-dir '/path/to/my/results'
    """
    main()